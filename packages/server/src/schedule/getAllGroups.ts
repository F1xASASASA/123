...
export default async function () {
	...
	for (const grade of grades) {
		...
		for (const group of Object.values(body).map(
			...
			const groupSchedule = await (
				await fetch(
					<string>Bun.env.BITRIX_URL +
						`mobile/teacher/schedule/spo_and_vo.php?name=` +
						group,
					{
						method: "GET",
						headers: {
			...
			if (groupSchedule.includes("Выберите подгруппу")) {
				console.log(`Group ${group} has subgroups!`);
	...
	console.log("Groups parsed!");

	redis.set("groups", JSON.stringify(groups, undefined, 4));
	...
	redis.set("teachers", JSON.stringify(teachers, undefined, 4));
