...
export default async function (login: string, password: string): Promise<Session> {
    ...
    if (authResponse.status !== 200) throw new Error("Invalid auth status: " + authResponse.status)
    ...
    for (let group of body.matchAll(/\(window\.BX\|\|top\.BX\)\.message\((.*)\)/gm)) {
